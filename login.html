<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>QuickCut - Login</title>
    <!-- Bootstrap 5 CSS -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <!-- Font Awesome Icons -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <!-- Custom CSS -->
    <link rel="stylesheet" href="css/login.css">
</head>
<body id="login-body">
    <!-- Navigation -->
    <nav class="navbar navbar-expand-lg navbar-dark fixed-top" id="login-nav">
        <div class="container">
            <a class="navbar-brand logo" href="welcome.html" id="login-logo">
                <i class="fas fa-cut me-2"></i>QuickCut
            </a>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav" id="login-menu-toggle">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto" id="login-nav-menu">
                    <li class="nav-item">
                        <a class="nav-link" href="welcome.html" id="nav-home"><i class="fas fa-home me-1"></i>Home</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="login.html" id="nav-book-appointment"><i class="fas fa-calendar-alt me-1"></i>Book Appointment</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="login.html" id="nav-queue-status"><i class="fas fa-list-ol me-1"></i>Queue Status</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="aboutus.html" id="nav-about"><i class="fas fa-info-circle me-1"></i>About</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link active" href="login.html" id="nav-login"><i class="fas fa-sign-in-alt me-1"></i>Login</a>
                    </li>
                </ul>
                <a href="login.html" class="btn btn-primary ms-lg-3 mt-2 mt-lg-0 book-now-btn" id="login-book-now">
                    <i class="fas fa-scissors me-1"></i>Book Now
                </a>
            </div>
        </div>
    </nav>

    <!-- Main Content -->
    <main class="container mt-5 pt-5" id="login-main">
        <div class="row align-items-center min-vh-75">
            <!-- Login Form Column -->
            <div class="col-lg-6 mb-5 mb-lg-0" id="login-form-column">
                <div class="card login-card shadow-lg" id="login-card">
                    <div class="card-body p-4 p-md-5">
                        <div class="text-center mb-4">
                            <h2 class="fw-bold text-primary" id="login-title">Welcome Back</h2>
                            <p class="text-muted" id="login-subtitle">Log in to manage your appointments</p>
                        </div>
                        
                        <form action="bookappointment.html" method="get" id="login-form">
                            <div class="mb-3">
                                <label for="email" class="form-label fw-semibold">Email Address</label>
                                <div class="input-group">
                                    <span class="input-group-text" id="email-icon">
                                        <i class="fas fa-envelope"></i>
                                    </span>
                                    <input type="email" class="form-control" id="login-email" placeholder="your@email.com" required>
                                </div>
                                <div class="invalid-feedback" id="email-error"></div>
                            </div>
                            
                            <div class="mb-3">
                                <label for="password" class="form-label fw-semibold">Password</label>
                                <div class="input-group">
                                    <span class="input-group-text" id="password-icon">
                                        <i class="fas fa-lock"></i>
                                    </span>
                                    <input type="password" class="form-control" id="login-password" required>
                                    <button type="button" class="btn btn-outline-secondary" id="toggle-password">
                                        <i class="fas fa-eye"></i>
                                    </button>
                                </div>
                                <div class="text-center mt-2">
                                    <a href="#" class="text-decoration-none text-primary forgot-password" id="forgot-password-link">
                                        Forgot Password?
                                    </a>
                                </div>
                                <div class="invalid-feedback" id="password-error"></div>
                            </div>
                            
                            <button type="submit" class="btn btn-primary btn-lg w-100 mt-3 login-btn" id="login-submit-btn">
                                <i class="fas fa-sign-in-alt me-2"></i>Log In
                            </button>
                            
                            <div class="text-center mt-3" id="login-message"></div>
                        </form>
                        
                        <div class="text-center mt-4 pt-3 border-top" id="create-account-section">
                            <p class="text-muted mb-3" id="new-user-text">New to QuickCut?</p>
                            <a href="create.html" class="btn btn-outline-primary btn-lg w-100 create-account-btn" id="create-account-btn">
                                <i class="fas fa-user-plus me-2"></i>Create an Account
                            </a>
                        </div>
                        
                        <div class="text-center mt-4" id="terms-section">
                            <p class="small text-muted" id="terms-text">
                                By logging in, you agree to our 
                                <a href="#" class="text-decoration-none text-primary" id="terms-link">Terms of Service</a> 
                                and 
                                <a href="#" class="text-decoration-none text-primary" id="privacy-link">Privacy Policy</a>
                            </p>
                        </div>
                    </div>
                </div>
            </div>
            
            <!-- Welcome Column -->
            <div class="col-lg-6" id="welcome-column">
                <div class="text-center text-lg-start">
                    <h1 class="display-4 fw-bold mb-3" id="welcome-title">
                        Skip the wait. 
                        <span class="text-primary">Get the cut.</span>
                    </h1>
                    <p class="lead text-muted mb-4" id="welcome-description">
                        Book your appointment online and avoid long queues. 
                        QuickCut connects you with the best barbers and stylists in your area.
                    </p>
                    
                    <div class="features mt-5" id="features-section">
                        <div class="row g-4">
                            <div class="col-md-6">
                                <div class="feature-card p-3 h-100" id="feature-1">
                                    <div class="feature-icon mb-3">
                                        <i class="fas fa-clock fa-2x text-primary"></i>
                                    </div>
                                    <h5 class="fw-bold" id="feature-1-title">Quick Booking</h5>
                                    <p class="text-muted small" id="feature-1-desc">Book appointments in under 2 minutes</p>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="feature-card p-3 h-100" id="feature-2">
                                    <div class="feature-icon mb-3">
                                        <i class="fas fa-calendar-check fa-2x text-primary"></i>
                                    </div>
                                    <h5 class="fw-bold" id="feature-2-title">Real-time Updates</h5>
                                    <p class="text-muted small" id="feature-2-desc">Live queue status and notifications</p>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="feature-card p-3 h-100" id="feature-3">
                                    <div class="feature-icon mb-3">
                                        <i class="fas fa-star fa-2x text-primary"></i>
                                    </div>
                                    <h5 class="fw-bold" id="feature-3-title">Top Professionals</h5>
                                    <p class="text-muted small" id="feature-3-desc">Rated and reviewed barbers & stylists</p>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="feature-card p-3 h-100" id="feature-4">
                                    <div class="feature-icon mb-3">
                                        <i class="fas fa-credit-card fa-2x text-primary"></i>
                                    </div>
                                    <h5 class="fw-bold" id="feature-4-title">Secure Payments</h5>
                                    <p class="text-muted small" id="feature-4-desc">Safe and easy online payments</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </main>

    <!-- Footer -->
    <footer class="footer-section" id="login-footer">
        <div class="container py-5">
            <div class="row">
                <div class="col-lg-4 mb-4 mb-lg-0" id="footer-left">
                    <h5 class="fw-bold mb-3" id="footer-logo">
                        <i class="fas fa-cut me-2 text-primary"></i>QuickCut
                    </h5>
                    <p class="text-light small" id="footer-description">
                        Skip the wait. Get the cut. QuickCut makes hair appointments fast, easy, and convenient.
                    </p>
                    <div class="social-links mt-3" id="footer-social">
                        <a href="https://facebook.com" target="_blank" class="text-white me-3" id="footer-facebook">
                            <i class="fab fa-facebook fa-lg"></i>
                        </a>
                        <a href="https://twitter.com" target="_blank" class="text-white me-3" id="footer-twitter">
                            <i class="fab fa-twitter fa-lg"></i>
                        </a>
                        <a href="https://instagram.com" target="_blank" class="text-white me-3" id="footer-instagram">
                            <i class="fab fa-instagram fa-lg"></i>
                        </a>
                        <a href="https://tiktok.com" target="_blank" class="text-white" id="footer-tiktok">
                            <i class="fab fa-tiktok fa-lg"></i>
                        </a>
                    </div>
                </div>
                
                <div class="col-lg-4 mb-4 mb-lg-0" id="footer-middle">
                    <h5 class="fw-bold mb-3">Contact Us</h5>
                    <ul class="list-unstyled">
                        <li class="mb-2" id="contact-phone">
                            <i class="fas fa-phone me-2 text-primary"></i>
                            (+251) 921456765
                        </li>
                        <li class="mb-2" id="contact-email">
                            <i class="fas fa-envelope me-2 text-primary"></i>
                            quickcut@gmail.com
                        </li>
                        <li id="contact-address">
                            <i class="fas fa-map-marker-alt me-2 text-primary"></i>
                            123 Main Street, Downtown
                        </li>
                    </ul>
                </div>
                
                <div class="col-lg-4" id="footer-right">
                    <h5 class="fw-bold mb-3">Working Hours</h5>
                    <ul class="list-unstyled">
                        <li class="mb-2">Monday - Sunday: 6:00 AM - 6:00 PM</li>
                    </ul>
                </div>
            </div>
            
            <hr class="bg-light my-4" id="footer-divider">
            
            <div class="text-center" id="footer-bottom">
                <p class="mb-0" id="copyright">
                    © 2025 QuickCut. All rights reserved. | 
                    <a href="#" class="text-decoration-none text-primary" id="privacy-policy">Privacy Policy</a> | 
                    <a href="#" class="text-decoration-none text-primary" id="terms-service">Terms of Service</a>
                </p>
            </div>
        </div>
    </footer>
    
    <!-- Back to Top Button -->
    <button class="btn btn-primary scroll-to-top" style="display: none;" id="back-to-top">
        <i class="fas fa-chevron-up"></i>
    </button>

    <!-- Bootstrap JS Bundle -->
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
    <!-- Auth (logout handling) -->
    <script src="js/auth.js"></script>
    <!-- Custom JavaScript -->
    <script src="js/login.js"></script>
</body>
</html>